...
#python neural_style_tutorial_2.py --height 250 --factor 0.25 --input "Emma_Watson.jpg" --style "vangogh_starry_night.jpg" --mask_img "Emma_Watson_mask.jpg"
parser = argparse.ArgumentParser(description='PyTorch Gentle Neural Style Transfer for Faces')
parser.add_argument('--height', type=int, default=-1, metavar='H',
                    help="image height")
parser.add_argument('--width', type=int, default=-1, metavar='W',
                    help="image width")
parser.add_argument('--factor', type=float, default=0, metavar='FC',
                    help="factor of making style less pronounced on mask")
parser.add_argument('--mask_img', type=str, default='mask', metavar='M',
                    help="mask image")
...
def normalize_tensor_mask(t):
    ...
    for i in range(t.size()[1]):
        for j in range(t.size()[2]):
            ...
            tot+=1
            if(t[0,i,j].data.cpu().numpy()==0)and (t[1,i,j].data.cpu().numpy()==0) and (t[2,i,j].data.cpu().numpy()==0):
                t.data[0, i, j] = 0
                t.data[1, i, j] = 0
                t.data[2, i, j] = 0
                cnt+=1
...
masks=[]
max_pooler=nn.MaxPool2d(kernel_size=2,stride=2)
if use_cuda:
    max_pooler=max_pooler.cuda()
...
def image_mask_loader(image_name):
    image = Image.open(image_name)
...
def generate_mask(prev_mask,inp):
    print("sizes: ",prev_mask.size(),inp.size())
    if (prev_mask.size()[2]!=inp.size()[2]):
        prev_mask=max_pooler(prev_mask)
    prev_mask = prev_mask.squeeze().permute(1,2,0)
    inp = inp.squeeze().permute(1,2,0)
    new_mask=torch.ones(inp.size())
    if use_cuda:
    ...
    for i in range(prev_mask.size()[0]):
        for j in range(prev_mask.size()[1]):
            cnt=0
            total+=1
            for k in range(prev_mask.size()[2]):
                if (prev_mask[i][j][k]).data.cpu().numpy()==0:
                    continue
                else:
                    cnt+=1
            if cnt==0:
                ...
                changed+=1
                if use_cuda:
                    new_mask[i][j]=torch.zeros(inp.size()[2]).cuda()
                else:
                    new_mask[i][j] = torch.zeros(inp.size()[2])

    print(str(changed)+"/"+str(total)+" masked")
    new_mask=new_mask.permute(2,0,1)
    new_mask=new_mask.unsqueeze(0)
    masks.append(Variable(new_mask).clone())
    if factor==0:
        return Variable(new_mask)
    else:
        new_mask = new_mask.squeeze().permute(1, 2, 0)
        while len(indices)>0:
            t=indices.pop()
            new_mask[(t)]+=factor
        new_mask = new_mask.permute(2, 0, 1)
...
mask_img = image_mask_loader(args.mask_img).type(dtype)
mask_img=normalize_tensor_mask(mask_img)

masks.append(mask_img.clone())
...
class StyleLoss(nn.Module):
    def __init__(self, target, weight, mask):
...
def get_style_model_and_losses(cnn, style_img, content_img, mask_img,
    ...
    for layer in list(cnn):
        if isinstance(layer, nn.Conv2d):
            ...
            if name in style_layers:
                ...
                mask=generate_mask(masks[-1],target_feature)
                style_loss = StyleLoss(target_feature_gram, style_weight, mask)
...
def run_style_transfer(cnn, content_img, style_img, mask_img, input_img, num_steps=200,
                       style_weight=10000, content_weight=10):
    """Run the style transfer."""
    print('Building the style transfer model..')
    model, style_losses, content_losses = get_style_model_and_losses(cnn,
                                                                     style_img, content_img, mask_img, style_weight,
                                                                     content_weight)
    ...
    while run[0] <= num_steps:
        ...
        def closure():
            ...
            if run[0] % 50 == 0:
                ...
                print()
                imshow(input_param.data, title=args.input.split('.')[0]+"_fac"+str(factor)+"_iter"+str(run[0]))
...
output = run_style_transfer(cnn, content_img, style_img, mask_img, input_img,args.num_iter)

# plt.figure()
imshow(output, title='Output Image_'+"_fac"+str(factor)+args.input.split('.')[0])
